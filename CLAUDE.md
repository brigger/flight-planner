...
Flask + Gunicorn, MySQL connector to the shared MariaDB container. Five tables: `users` (id, email, first_name, last_name, email_verified_at, last_login_at), `magic_tokens` (one-shot email-verification / sign-in links, 24 h for verify, 1 h for login), `sessions` (90-day cookie tokens), `flight_plans` (`id, user_id, name, aircraft, updated_at, archived_at, plan_json`; UNIQUE `(user_id, name)`; `archived_at` set = read-only — content updates and renames return 423 until de-archived via `PUT /api/plans/:id/archive` `{archived: bool}`, admin variant `PUT /api/admin/plans/:id/archive`), `freq_directory` (global waypoint-name → frequency list feeding the NAV Plan Freq/Waypoint dropdown; seeded on first boot when empty, administered in the `velis_admin.html` "Waypoint frequencies" card; `GET /api/freqs` is public, `POST/PUT/DELETE /api/freqs[/:id]` are admin-only). Auth is passwordless: register with first/last/email → email contains a magic link → `/api/auth/verify/<token>` sets an HttpOnly session cookie and returns a branded boarding-pass landing page. Endpoints: `POST /api/auth/register`, `POST /api/auth/login`, `GET /api/auth/verify/<token>`, `GET /api/auth/me`, `POST /api/auth/logout`, `GET /api/ping`, `GET /api/plans`, `GET /api/plans/:id`, `POST /api/plans`, `PUT /api/plans/:id`, `PUT /api/plans/:id/archive`, `DELETE /api/plans/:id`. Plan endpoints require the session cookie and scope rows to the owning user. Duplicate `(user_id, name)` → 409. Full detail (env vars, SMTP, public URL) in `backend/README.md`.
