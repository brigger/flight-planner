...
`127.0.0.1:8003`, reverse-proxied by Nginx at `/flight-planner/api/`.
...
| `PUBLIC_URL` | yes | absolute origin + path prefix that the magic link should resolve to — e.g. `https://example.com/flight-planner`. Email links are built as `{PUBLIC_URL}/api/auth/verify/<token>` |
